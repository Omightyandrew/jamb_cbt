(async function(){
  ...
  function render(){
    ...
    list.innerHTML=visibleRows.map(row=>{
      ...
      return `<article class="question-card"><div class="question-meta"><span>${escapeHtml(row.subject)}</span><span>${row.type==='past'?'Past Question':'Practice Question'}</span><span>${escapeHtml(topicOf(row))}</span><span>Question ${row.index}</span></div><h2>${escapeHtml(row.question)}</h2><button type="button" class="ai-question-button" data-ai-question-id="${escapeHtml(row.id)}">Ask AI about this question</button><div class="options">${optionsMarkup}</div>${explanationMarkup}</article>`;
    }).join('');
